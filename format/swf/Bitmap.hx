   ...
   */
   ...
   public function new(inStream:SWFStream,inLossless:Bool,inVersion:Int)
   {
      ...
      if (inLossless)
      {
         ...
         var w = inStream.readUInt16();
         var h = inStream.readUInt16();
         var table_size = fmt==3 ? (inStream.readByte()+1) : 0;
         
         var buffer:flash.utils.ByteArray = inStream.readFlashBytes(inStream.getBytesLeft());
         buffer.uncompress();
      ...
      else
      {
         ...
         if (inVersion==2)
         {
            var size = inStream.getBytesLeft();
         ...
         else if (inVersion==3)
         {
            ...
            alpha = inStream.readFlashBytes(inStream.getBytesLeft());
            alpha.uncompress();
         }
